...
mkValidator dat () ctx = traceIfFalse "Beneficiary 1 signature missing" signedByBeneficiary1 &&
                         traceIfFalse "deadline passed already - cannot transact" deadlineHasReached || 
                         traceIfFalse "Beneficiary 2 signature missing" signedByBeneficiary2 &&
                         traceIfFalse "deadline has not reached - cannot transact" deadlineNotReached 
  ...
  where
    ...
    deadlineHasReached :: Bool
    deadlineHasReached = contains (from $ deadline dat) $ txInfoValidRange info

    deadlineNotReached :: Bool
    deadlineNotReached = not $ contains (from $ deadline dat) $ txInfoValidRange info
