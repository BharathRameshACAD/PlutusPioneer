...
mkPolicy pkh deadline () ctx = traceIfFalse "Transaction not signed" txSignedByfunc &&
                               traceIfFalse "Deadline has already passed" checkDeadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    txSignedByfunc :: Bool
    txSignedByfunc = txSignedBy info pkh

    checkDeadline :: Bool
    -- checkDeadline = not $ contains (from deadline) $ txInfoValidRange info
    checkDeadline = contains (to deadline) $ txInfoValidRange info
